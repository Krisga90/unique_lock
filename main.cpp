...
int main() {
  ...
  std::thread thr1(printWithUniqueLockV2, std::cref(text), throw_except_argument);
  std::thread thr2(printWithUniqueLockV2, std::cref(text), throw_except_argument);
  std::thread thr3(printWithUniqueLockV2, std::cref(text), throw_except_argument);
...
void printWithUniqueLock(const std::string& str, bool throw_exc)
{
  try
  {
    std::unique_lock u_lock(PRINT_MUTEX);
...
void printWithUniqueLockV2(const std::string& str, bool throw_exc)
{
  try
  {
    std::unique_lock u_lock(PRINT_MUTEX, std::defer_lock);
    u_lock.lock();
    std::unique_lock u_lock2(PRINT_MUTEX, std::adopt_lock);
    for(char c : str)
    {
      std::cout << c;
    }
